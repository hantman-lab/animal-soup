...
def deconv(in_planes: int, out_planes: int, bias: bool = True):
    ...
    return nn.Sequential(
        nn.ConvTranspose2d(
            in_planes, out_planes, kernel_size=4, stride=2, padding=1, bias=bias
        ),
        nn.LeakyReLU(0.1, inplace=True),
    )
...
def predict_flow(in_planes: int, out_planes: int = 2, bias: bool = False):
    ...
    return nn.Conv2d(
        in_planes, out_planes, kernel_size=3, stride=1, padding=1, bias=bias
    )
...
class CropConcat(nn.Module):
    ...
    def forward(self, tensors: tuple) -> torch.Tensor:
        ...
        hs, ws = [tensor.size(-2) for tensor in tensors], [
            tensor.size(-1) for tensor in tensors
        ]
        h, w = min(hs), min(ws)

        return torch.cat(
            tuple([tensor[..., :h, :w] for tensor in tensors]), dim=self.dim
        )
...
def conv3d(
    ...
    if padding is None and type(kernel_size) == int:
        ...
        raise ValueError(
            "Unknown padding type {} and kernel_size type: {}".format(
                padding, kernel_size
            )
        )
