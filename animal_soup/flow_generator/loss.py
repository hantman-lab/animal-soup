...
class MotionNetLoss(torch.nn.Module):
    ...
    def forward(
        self, originals, images, reconstructed, outputs, model: torch.nn.Module
    ):
        """
        ...
        if outputs[0].size(0) != images[0].size(0):
            raise ValueError(
                "Image shape: ", images[0].shape, "Flow shape:", outputs[0].shape
            )
        if self.is_multiscale:
            ...
            if len(images) == 1:
                ...
                raise ValueError(
                    "Incorrect number of multiscale outputs: %d. Expected %d"
        ...
        L1s = [
            charbonnier(real - estimated, alpha=0.4)
            for real, estimated in zip(images, reconstructed)
        ]
        ...
        smooth_mean = [
            torch.mean(i, dim=[1, 2, 3]) * weight for i, weight in zip(smooths, weights)
        ]

        SSIMs = [
            self.ssim(real, estimated) for real, estimated in zip(images, reconstructed)
        ]
        ...
        if self.flow_sparsity:
            ...
            flow_l1s = [
                torch.mean(torch.abs(i), dim=[1, 2, 3]) * self.sparsity_weight
                for i in outputs
            ]
...
def charbonnier_smoothness(flows, alpha=0.3, eps=1e-7):
    return charbonnier(gradient_x(flows), alpha=alpha, eps=eps) + charbonnier(
        gradient_y(flows), alpha=alpha, eps=eps
    )
